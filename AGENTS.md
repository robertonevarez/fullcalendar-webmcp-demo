...
- **Experiment (unreleased):** a subset of seeds projects host-selected `metadata` (location / attendees / team) for WebMCP agents. Private seed fields (`tenantId`, `billingCode`, `privateNotes`) are never written into `CalendarEvent.metadata`. Persistence key is `…:v3:2026-sep-nov`. No calendar UI chrome was added for metadata.
